...
async def test():
    ...
    async with AsyncSession(impersonate="chrome124") as sess:
        ...
        print("=== Fetching content page with curl_cffi (chrome124) ===")
        resp = await sess.get(content_url, headers=pw_headers, cookies=cookies)
        ...
        if "keygenerate" in resp.url.lower():
            ...
            if mt:
                print(f"MEDIA_TOKEN found: {mt.group(1)[:80]}...")
            pw = re.search(r'PW_HEADERS\s*=\s*({.*?});', body, re.DOTALL)
            if pw:
                print("PW_HEADERS found in page")
            # save for verification
            with open("_debug_curl_result.html", "w", encoding="utf-8") as f:
                f.write(body)
                print("Saved page to _debug_curl_result.html")
